...
function setDownloadStatus(patch) {
	state.downloadStatus = Object.assign({}, state.downloadStatus, patch)
}

function beginDownloadStatus(version, asset, total) {
	setDownloadStatus({
		active: true,
		done: false,
		error: '',
		...
		current: 0,
		total: Number(total) || Number(asset.size) || 0,
		progress: 0
	})
}

function completeDownloadStatus(filePath) {
	setDownloadStatus({
		active: false,
		done: true,
		error: '',
...
function failDownloadStatus(err) {
	setDownloadStatus({
		active: false,
		done: false,
...
function getDownloadStatus() {
	return Object.assign({}, state.downloadStatus)
}

async function writeResponseToFile(response, targetPath, onProgress) {
	const partialPath = targetPath + '.partial'
	let current = 0
	const total = Number(response.headers.get('content-length')) || 0
	fs.rmSync(partialPath, { force: true })
	const output = fs.createWriteStream(partialPath)
	const outputError = new Promise((resolve, reject) => {
...
async function downloadAsset(asset, targetPath, onProgress) {
	...
	await writeResponseToFile(response, targetPath, onProgress)
...
async function downloadLatestUpdate() {
	...
	state.downloadInFlight = (async () => {
		...
		if (!isCompleteDownloadedAsset(filePath, asset)) {
			beginDownloadStatus(version, asset, asset.size)
			...
			try {
				await downloadAsset(asset, filePath, (current, responseTotal) => {
					const total = Number(asset.size) || responseTotal || 0
					setDownloadStatus({
						current,
						total,
				...
				completeDownloadStatus(filePath)
			} catch (err) {
				failDownloadStatus(err)
				throw err
			}
		} else {
			const stats = fs.statSync(filePath)
