...
const download = {
    ...
    get: (name, url, streamId, filenameCb, metaUrl, metaId, metaType) => {
        ;(async () => {
            ...
            const filename = removeIllegalCharacters((filenameDecision || {}).filename)
            if (!filenameDecision || !filename) {
            ...
            logger.info('Starting download', {
                url,
                filename,
                filenameReason: filenameDecision.reason,
                type,
                finalUrl: headers.finalUrl,
                contentDisposition: headers.contentDisposition,
                streamId,
            ...
            if (type && hlsTypes.includes(normalizeContentType(type))) {
                ...
                writeStream.on('close', () => {
                    const idx = download.findIdx(url)
                    if (idx > -1) {
                        if (files[idx].current < files[idx].total && !files[idx].stopped && !files[idx].error) {
