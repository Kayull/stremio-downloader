...
async function isDownloaderReachable(urls) {
	...
	try {
		...
		if (
			...
			typeof payload.hasFolder !== 'boolean' ||
			typeof payload.useShowSubfolders !== 'boolean'
		)
			return false

		const releaseInfoResponse = await fetch(urls.baseUrl + '/api?method=release-info', {
			signal: controller.signal
		})
		...
		if (!releaseInfo || typeof releaseInfo.currentVersion !== 'string' || typeof releaseInfo.updateAvailable !== 'boolean')
			return false
