...
const shellStorage = require('./shellStorage')
...
const ENGINE_STATUS_URL_CANDIDATES = [
	'http://127.0.0.1:11470/settings',
	'http://localhost:11470/settings',
...
async function requestEngineStatus(targetUrl) {
...
async function detectEngineStatus() {
	let lastError = null

	for (const targetUrl of ENGINE_STATUS_URL_CANDIDATES) {
		const result = await requestEngineStatus(targetUrl)
		if (result.running)
			return result
...
function getShellStorageSummaryMessage(summary, baseAction) {
	if (!summary.restoringLoginState)
		return baseAction + ' without login state'
	if (summary.loginStateStoredOnDisk)
		return baseAction + ' with persisted login state'
	return baseAction + ' with in-memory login state'
}

function sendApiError(res, statusCode, err, fallbackMessage) {
	sendJson(res, statusCode, {
		done: false,
		error: err.code || 'request_failed',
		message: err.message || fallbackMessage || 'The request could not be completed.'
	})
}

function isDesktopUpdateRequest(query) {
	return query.desktop === 'true'
}

function scheduleUpdateShutdown() {
	setTimeout(() => {
		if (typeof shutdownHandler === 'function') {
			shutdownHandler(0, 'install_update')
			return
		}
...
module.exports = {
	...
	router: async (req, res) => {
		...
		if (query.method == 'add-download') {
			...
			if (!downDir) {
				sendJson(res, 400, {
					...
					error: 'missing_download_folder',
					message: 'No download folder has been configured yet.'
				})
				return
			}
			...
			try {
				...
				const savedFolder = downloadDir.set(folder)
				sendJson(res, 200, { done: true, folder: savedFolder })
			} catch (err) {
				logger.warn('change-folder failed', err)
				sendJson(res, 200, {
					done: false,
					error: err.code || 'picker_failed',
					message: err.message || (err.code === 'invalid_download_folder'
						? 'The app folder cannot be used as the download folder.'
		...
		} else if (query.method == 'download-update') {
			if (!isDesktopUpdateRequest(query)) {
				sendJson(res, 400, {
					done: false,
					error: 'desktop_only',
					message: 'Automatic update downloads are only available in the desktop app.'
				})
				return
			}

			try {
				const result = await appUpdate.downloadLatestUpdate()
				sendJson(res, 200, result)
			} catch (err) {
				logger.warn('download-update failed', err)
				sendApiError(res, 500, err, 'Could not download the update.')
			}
		} else if (query.method == 'update-download-status') {
			if (!isDesktopUpdateRequest(query)) {
				sendJson(res, 400, {
					done: false,
					error: 'desktop_only',
			...
			sendJson(res, 200, appUpdate.getDownloadStatus())
		} else if (query.method == 'install-downloaded-update') {
			if (!isDesktopUpdateRequest(query)) {
				sendJson(res, 400, {
					done: false,
					error: 'desktop_only',
					message: 'Automatic update installation is only available in the desktop app.'
				})
				return
			}

			try {
				const result = await appUpdate.prepareInstallAfterQuit()
				sendJson(res, 200, result)
				scheduleUpdateShutdown()
			} catch (err) {
				logger.warn('install-downloaded-update failed', err)
				sendApiError(res, 500, err, 'Could not prepare the update installer.')
			}
		} else if (query.method == 'engine-status') {
			const engineStatus = await detectEngineStatus()
			sendJson(res, 200, {
				running: !!engineStatus.running,
				url: engineStatus.url || '',
				status: Number.isInteger(engineStatus.status) ? engineStatus.status : null
			})
			...
			sendJson(res, 200, { done: true })
		} else if (query.method == 'set-auto-delete-empty-show-folders') {
			sendJson(res, 200, {
				done: true,
				autoDeleteEmptyShowFolders: downloadDir.setAutoDeleteEmptyShowFolders(query.enabled === 'true')
			})
		} else if (query.method == 'set-persist-login-state') {
			const persistLoginState = downloadDir.setPersistLoginState(query.enabled !== 'false')
			if (!persistLoginState)
				shellStorage.clearSnapshot('shell_storage_disabled')
			sendJson(res, 200, {
				done: true,
				persistLoginState
			})
		} else if (query.method == 'reset-login-state') {
			const result = shellStorage.clearLoginState('manual_reset')
			sendJson(res, 200, {
				done: true,
		...
		} else if (query.method == 'shell-storage') {
			if (req.method === 'GET') {
				const snapshot = shellStorage.read()
				const summary = shellStorage.getSummary(snapshot, {
					path: shellStorage.getPath()
				})
				if (summary.restoringLoginState) {
					logger.info(getShellStorageSummaryMessage(summary, 'Restoring shell storage snapshot'), summary)
				}
			...
			if (req.method === 'POST') {
				try {
					...
					const result = shellStorage.write(payload)
					sendJson(res, 200, {
						done: true,
						changed: result.changed
					})
				} catch (err) {
					logger.warn('shell-storage update failed', err)
					sendJson(res, 400, {
						done: false,
						error: 'invalid_payload',
						message: 'Could not parse shell storage payload.'
					})
				}
				return
			}
			...
			sendJson(res, 405, {
				...
				message: 'Unsupported shell storage request method.'
			})
