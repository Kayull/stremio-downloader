	...
	router: async (req, res) => {
		...
		if (query.method == 'add-download') {
			...
			if (req.method === 'GET') {
				...
				if (summary.entries > 0 || summary.inMemoryAuthKeys.length > 0) {
					logger.info(summary.restoringLoginState
						? 'Restoring shell storage snapshot with in-memory login state'
						: 'Restoring shell storage snapshot without login state', summary)
				}
			...
			if (req.method === 'POST') {
				try {
					...
					if (result.changed) {
						const summary = shellStorage.getSummary(result.snapshot, {
							path: shellStorage.getPath()
						})
						logger.info(summary.restoringLoginState
							? 'Updated shell storage snapshot and kept login state in memory'
							: 'Updated shell storage snapshot without login state', summary)
					}
					sendJson(res, 200, {
						done: true,
