...
function isDebridPlaybackUrl(url) {
	if (!url || typeof url !== 'string')
		return false

	try {
		const parsed = new URL(url)
		return parsed.pathname.includes('/debrid/playback/')
	} catch (err) {
		return url.includes('/debrid/playback/')
	}
}
...
function getDownloadSourceKind(url, contentType) {
	...
	if (isDebridPlaybackUrl(url))
		return 'debrid-playback'
	if (contentType && [
		'video/m3u',
		'video/m3u8',
		'video/hls',
