...
function getSnapshotSummary(snapshot, options) {
	const normalizedSnapshot = normalizeSnapshot(snapshot)
	const persistedSnapshot = removeAuthKeys(normalizedSnapshot)
	const inMemoryAuthSnapshot = extractAuthKeys(normalizedSnapshot)
	const authKeys = listPresentAuthKeys(inMemoryAuthSnapshot)
	const summary = {
		...
		inMemoryAuthKeys: authKeys,
		restoringLoginState: authKeys.length > 0,
		loginStateStoredInMemoryOnly: authKeys.length > 0
	}
...
function logSnapshotDiff(previousSnapshot, nextSnapshot) {
	const diff = getSnapshotDiff(previousSnapshot, nextSnapshot)
	if (!countDiffEntries(diff))
		return

	logger.info('Shell storage key changes', {
		localAdded: diff.localStorage.added,
		localRemoved: diff.localStorage.removed,
		localChanged: diff.localStorage.changed,
		sessionAdded: diff.sessionStorage.added,
		sessionRemoved: diff.sessionStorage.removed,
		sessionChanged: diff.sessionStorage.changed,
		authRelatedKeys: listInterestingKeys(diff)
	})
}
...
function clearSnapshot(reason) {
	...
	if (listPresentAuthKeys(previousVolatileSnapshot).length) {
		logger.info('Cleared in-memory shell login state', {
...
function writeSnapshot(snapshot) {
	...
	const normalizedSnapshot = normalizeSnapshot(snapshot)
	const persistedSnapshot = removeAuthKeys(normalizedSnapshot)
	volatileSnapshot = extractAuthKeys(normalizedSnapshot)
	const effectiveSnapshot = mergeSnapshots(persistedSnapshot, volatileSnapshot)
	const previousAuthKeys = listPresentAuthKeys(previousVolatileSnapshot)
	const currentAuthKeys = listPresentAuthKeys(volatileSnapshot)

	if (!previousAuthKeys.length && currentAuthKeys.length) {
		logger.info('Detected in-memory shell login state', {
	...
	logSnapshotDiff(previousSnapshot, effectiveSnapshot)
