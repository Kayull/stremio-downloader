...
function buildRequestHeaders(req, opts, targetUrl) {
	...
	const cookieHeader = cookieJar.getCookieHeader(targetUrl, req.headers.cookie)
	if (cookieHeader)
...
function shouldRewriteToGet(statusCode, method) {
...
async function fetchWithRedirects(req, opts, targetUrl, controller) {
	let currentUrl = targetUrl
	let currentMethod = req.method
	let currentBody

	if (!['GET', 'HEAD'].includes(req.method))
		currentBody = await readRequestBody(req)

	for (let redirectCount = 0; redirectCount < 10; redirectCount++) {
		const upstream = await fetch(currentUrl, {
			method: currentMethod,
			headers: buildRequestHeaders(req, opts, currentUrl),
			body: currentBody,
			redirect: 'manual',
		...
		if (typeof upstream.headers.getSetCookie === 'function')
			cookieJar.storeFromResponse(currentUrl, upstream.headers.getSetCookie())
		...
		if (shouldRewriteToGet(upstream.status, currentMethod)) {
			currentMethod = 'GET'
...
const proxify = {
	...
	createProxyServer: router => {
		router.all(/^\/web\/.*/, async (req, res) => {
			...
			try {
				const upstream = await fetchWithRedirects(req, opts, targetUrl, controller)
